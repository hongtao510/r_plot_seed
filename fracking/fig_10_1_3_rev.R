...
PD_1acre_ring_3_benzene <- filter(PD_1acre, RING=="R3_300FT", Data_type=="Max") %>% select(benzene)




# 10-1
ggplot(filter(Summary_sta_all_sub_melt, variable=="benzene", Data_type=="Max"), aes(y = logvalue, x =Location, fill =RING))+
      ...
      scale_fill_discrete(name = "AERMOD Ring", labels = legend_label_text) +
      theme_Publication(15)+
      guides(fill=guide_legend(nrow=2,byrow=TRUE))


ggplot(filter(Summary_sta_all_sub_melt, variable=="benzene", Data_type=="Max"), aes(y = value, x =Location, fill =RING))+
      stat_summary(fun.data = min.median.sd.max, geom="boxplot", position = position_dodge()) +
      # geom_boxplot()+
      labs(x = "", y = bquote('VOC Concentrations (scale, '*ug~ m^-3*')'), title="") +
      scale_fill_discrete(name = "AERMOD Ring", labels = legend_label_text) +
      theme_Publication(15)+
      guides(fill=guide_legend(nrow=2,byrow=TRUE))
...
ggplot(filter(Summary_sta_all_sub_melt, Data_type=="Max"), aes(y = logvalue, x =Location, fill =RING))+
      stat_summary(fun.data = min.median.sd.max, geom="boxplot",position = position_dodge()) +
      labs(x = "", y = bquote('VOC Concentrations (log10 scale, '*ug~ m^-3*')'), title="") +
      scale_y_continuous(limits = c(-5.0, 4.5), breaks = seq(-5.0, 4.5, by = 0.5))+
      scale_fill_discrete(name = "AERMOD Ring", labels = legend_label_text) +
